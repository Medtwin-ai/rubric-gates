...
\begin{itemize}[noitemsep]
    ...
    reference implementations for ICU cohort tasks. Primary evaluation.
    \item \textbf{Cross-domain datasets} spanning 7 clinical specialties:
    \begin{itemize}[noitemsep]
        \item Cardiology: PTB-XL (ECG classification)
        \item Neurology: CHB-MIT Seizure, EEG Motor Imagery
        \item Sleep Medicine: Sleep-EDF (polysomnography)
        \item Metabolic: Big Ideas Glycemic Wearable
        \item Diabetes: CGMacros (continuous glucose monitoring)
        \item Oncology: MIMIC-IV cancer cohort (ICD-10 codes C00--C97)
        \item Neonatal: Preterm Infant Cardio
    \end{itemize}
\end{itemize}
...
A key question is whether rubrics developed for ICU data generalize to other clinical 
domains. We evaluate transfer to four distinct domains using the same three-tier 
framework.

\begin{table}[t]
    \centering
    \caption{Cross-domain transfer of rubrics developed on ICU data (MIMIC-IV + eICU) 
    to other clinical domains. Tier 1 (constitutional) transfers perfectly; Tier 2 
    (clinical invariants) requires domain adaptation; Tier 3 (benchmarks) requires 
    domain-specific references.}
    \label{tab:transfer}
    \begin{tabular}{@{}llccc@{}}
        \toprule
        \textbf{Domain} & \textbf{Dataset} & \textbf{Tier 1} & \textbf{Tier 2} & \textbf{Tier 3} \\
        \midrule
        Cardiology/ECG & PTB-XL & 1.00 & 0.91 & 0.84 \\
        Neurology/EEG & CHB-MIT Seizure & 1.00 & 0.87 & 0.81 \\
        Sleep Medicine & Sleep-EDF & 1.00 & 0.88 & 0.79 \\
        Metabolic & Glycemic Wearable & 1.00 & 0.92 & 0.85 \\
        Diabetes/CGM & CGMacros & 1.00 & 0.93 & 0.86 \\
        Oncology & MIMIC-IV (ICD C*) & 1.00 & 0.89 & 0.77 \\
        Neonatal ICU & Preterm Cardio & 1.00 & 0.85 & 0.72 \\
        \bottomrule
    \end{tabular}
\end{table}

Table~\ref{tab:transfer} reveals important insights across seven clinical domains:

\begin{itemize}[noitemsep]
    \item \textbf{Tier 1 transfers perfectly:} Constitutional constraints (determinism, 
    auditability) are domain-agnostic and achieve 100\% pass rate across all domains.
    
    \item \textbf{Tier 2 requires adaptation:} Clinical invariants differ substantially:
    EEG amplitude ranges (neurology), glucose thresholds (metabolic/diabetes), tumor 
    staging criteria (oncology), and neonatal vital sign norms all require domain-specific 
    rubric libraries.
    
    \item \textbf{Tier 3 needs domain references:} Task benchmarks require new reference 
    implementations: seizure detection (neurology), arrhythmia classification (cardiology), 
    sleep staging (polysomnography), hypoglycemia prediction (metabolic), cancer cohort 
    selection (oncology).
    
    \item \textbf{Oncology and neonatal show largest gaps:} Cancer staging rubrics and 
    preterm infant physiology diverge most from adult ICU, requiring specialized Tier 2 
    definitions (0.77--0.72 Tier 3 pass rates vs. 0.85+ for other domains).
\end{itemize}

This validates our hierarchical design: universal principles (Tier 1) transfer freely, 
while domain knowledge (Tiers 2--3) requires structured adaptation.
...
PTB-XL, CHB-MIT Seizure, EEG Motor Imagery, Sleep-EDF, Big Ideas Glycemic 
Wearable, CGMacros, and Preterm Infant Cardio databases. The comprehensive 
multi-domain evaluation spanning cardiology, neurology, sleep medicine, 
metabolic disorders, diabetes, oncology, and neonatal care was made possible 
by PhysioNet's open data initiatives. We also thank the reviewers for their 
...
\begin{table}[h]
    ...
    \begin{tabular}{@{}llrr@{}}
        \toprule
        \textbf{Domain} & \textbf{Dataset} & \textbf{Records} & \textbf{Size} \\
        \midrule
        \multicolumn{4}{l}{\textit{Anchor Datasets (ICU)}} \\
        ICU & MIMIC-IV v2.2 & 299,712 patients & 7.2 GB \\
        ICU & eICU-CRD v2.0 & 139,367 patients & 5.2 GB \\
        \midrule
        \multicolumn{4}{l}{\textit{Cross-Domain Validation}} \\
        Cardiology & PTB-XL v1.0.3 & 21,837 ECGs & 7.0 GB \\
        Neurology & CHB-MIT Seizure & 23 patients & 5.0 GB \\
        Neurology & EEG Motor Imagery & 109 subjects & 3.4 GB \\
        Sleep & Sleep-EDF v1.0.0 & 197 recordings & 2.1 GB \\
        Metabolic & Glycemic Wearable & 16 subjects & 0.8 GB \\
        Diabetes & CGMacros & 89 subjects & 1.3 GB \\
        Oncology & MIMIC-IV (ICD C*) & 45,000+ patients & (subset) \\
        Neonatal & Preterm Infant Cardio & 10 infants & 0.5 GB \\
        \bottomrule
    \end{tabular}
    \caption{Dataset statistics across clinical domains. Oncology cohort extracted 
    from MIMIC-IV using ICD-10 cancer codes (C00--C97).}
\end{table}
