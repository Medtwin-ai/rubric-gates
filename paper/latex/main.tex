...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{Cross-domain datasets}: PTB-XL (cardiology/ECG), Sleep-EDF 
    (polysomnography), CGMacros (diabetes/CGM), Preterm Infant Cardio (neonatal). 
    Tests rubric generalization beyond ICU.
\end{itemize}
...
Table~\ref{tab:transfer} reveals important insights:
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{Tier 2 requires adaptation:} Clinical invariants (e.g., heart rate 
    ranges) differ between adult ICU and neonatal populations. Domain-specific rubric 
    libraries are necessary.
    
    \item \textbf{Tier 3 needs domain references:} Task benchmarks require new reference 
    implementations for each domain (e.g., arrhythmia detection for ECG, sleep staging 
    for polysomnography).
    
    \item \textbf{Neonatal shows largest gap:} Physiological norms differ substantially 
    for preterm infants, requiring specialized Tier 2 rubrics.
\end{itemize}
...
PTB-XL, Sleep-EDF, CGMacros, and Preterm Infant Cardio databases. The 
multi-domain evaluation was made possible by PhysioNet's open data initiatives. 
...
\begin{table}[h]
    ...
    \begin{tabular}{@{}llrr@{}}
        ...
        \bottomrule
    \end{tabular}
    \caption{Dataset statistics across clinical domains.}
\end{table}
